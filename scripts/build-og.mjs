...
/* ================= five-numbers share card (R30) ================= */
// The masthead's five cited figures, copied exactly — never reworded here.
// Same frame chrome as the section cards; regenerated by --cards-only so a
// data refresh can never strand a stale refresh date on the card.
const FIVE = [
  { value: '$100B', label: 'PRIVATE' },
  { value: '$12.5B', label: 'PUBLIC' },
  { value: '9,000', label: 'DIRECT JOBS' },
  { value: '~50,000', label: 'TOTAL' },
  { value: '232', label: 'MI' },
];
const FIVE_POS = [
  { x: 64, y: 216 },
...
const fiveFigures = FIVE.map((f, i) => {
  const { x, y } = FIVE_POS[i];
  return `<line x1="${x + 3}" y1="${y - 76}" x2="${x + 47}" y2="${y - 76}" stroke="${T.copper}" stroke-width="4"/>
  <text x="${x}" y="${y}" font-family="Archivo" font-weight="900" font-size="62" letter-spacing="-1" fill="${T.ink}">${esc(f.value)}</text>
  <text x="${x + 3}" y="${y + 32}" font-family="IBM Plex Mono" font-weight="500" font-size="17" letter-spacing="2" fill="${T.muted}">${esc(f.label)}</text>`;
}).join('\n  ');
const fiveSvg = `<svg width="1200" height="630" viewBox="0 0 1200 630" xmlns="http://www.w3.org/2000/svg">
  <rect width="1200" height="630" fill="${T.silicon}"/>
  <rect x="14.5" y="14.5" width="1171" height="601" fill="none" stroke="${T.ink}" stroke-width="1"/>
  <rect x="20.5" y="20.5" width="1159" height="589" fill="none" stroke="${T.hairline}" stroke-width="1"/>

  ${mapMotif({ x: 780, y: 130, w: 360, h: 240 }, { labels: false })}
  ...
  <line x1="64" y1="540" x2="700" y2="540" stroke="${T.ink}" stroke-width="1"/>
  <text x="64" y="570" font-family="IBM Plex Mono" font-size="14" letter-spacing="1.4" fill="${T.muted}">DATA REFRESHED ${refreshed} · ALL FIGURES CITED · ESTIMATES LABELED</text>
  <text x="64" y="594" font-family="IBM Plex Mono" font-size="14" letter-spacing="1.4" fill="${T.muted}">SECOND-CORRIDOR.VERCEL.APP</text>
</svg>`;
writeFileSync(new URL('public/og/five-numbers.png', root), renderPng(fiveSvg));

console.log(
  `section cards written: ${sections.length} + five-numbers.png → public/og/ (refreshed ${refreshed})`
);
