...
- [ ] R3 — repo public *(staged: history audited; flips with the checkpoint approval)*
...
- **R3 staged, not flipped:** this is the checkpoint wave — the PR review IS
  the gate the repo goes public behind. History audited: no secret-shaped
  strings in any committed blob, no .env/key/raw files ever committed. One
  finding for Alex: commit metadata (early commits + the refresh workflow's
  git config) carries `Alex Harris <aharris128@gmail.com>` — the public byline
  is "Alex" alone, so going public exposes the full name + personal email in
  history. Rewriting history would fix it but breaks tags/PR refs; Alex
  decides at the checkpoint. The flip itself is one API call on approval.
